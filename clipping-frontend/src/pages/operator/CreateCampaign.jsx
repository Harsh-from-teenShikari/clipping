...
import { Button, Input, Select, Alert, PageHeader, Card } from '../../components/UI'
...
export default function CreateCampaign() {
  ...
  const validateStep = () => {
    ...
    if (step === 2) {
      if (!form.reward_pool || isNaN(form.reward_pool) || Number(form.reward_pool) <= 0)
    ...
    try {
      const payload = {
        ...
        required_hashtags: form.required_hashtags
          ? form.required_hashtags.split(',').map(h => h.trim()).filter(Boolean)
          : [],
        banned_keywords: form.banned_keywords
          ? form.banned_keywords.split(',').map(k => k.trim()).filter(Boolean)
          : [],
      ...
      <div className={styles.page}>
        <div className={styles.successBox}>
          ...
          <p className={styles.successDesc}>
            <strong>{created.name}</strong> has been created as a draft. Activate it to make it visible to creators.
          </p>
          <div className={styles.successMeta}>
            <div className={styles.metaItem}>
              <span className={styles.metaLabel}>Status</span>
              <span className={styles.metaVal} style={{ color: 'var(--orange)' }}>Draft</span>
            </div>
            <div className={styles.metaItem}>
              <span className={styles.metaLabel}>Reward Pool</span>
              <span className={styles.metaVal} style={{ color: 'var(--accent)' }}>${created.reward_pool}</span>
            </div>
            <div className={styles.metaItem}>
          ...
          <div className={styles.successActions}>
            ...
            <Button variant="ghost" onClick={() => navigate('/operator/campaigns')}>
              Save as Draft
            </Button>
          </div>
        </div>
      ...
      <Card className={styles.formCard}>
        ...
        {step === 0 && (
          <div className={styles.formSection}>
            ...
            <p className={styles.stepDesc}>Set the name, type and platform for your campaign</p>
        ...
        {step === 1 && (
          <div className={styles.formSection}>
            ...
            <p className={styles.stepDesc}>Define who can apply and what content rules apply</p>
            <div className={styles.formGrid}>
              <Input
                label="Target Niche *"
            ...
            <div className={styles.formGrid}>
              <div>
                ...
                />
                <p className={styles.fieldHint}>Creators must include these in their posts</p>
              </div>
              <div>
                <Input
                ...
                />
                <p className={styles.fieldHint}>Posts containing these will be rejected</p>
              </div>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className={styles.formSection}>
            ...
            <p className={styles.stepDesc}>Set how much you're paying and what performance threshold to require</p>
            <div className={styles.formGrid}>
              <div>
                <Input
                ...
                />
                <p className={styles.fieldHint}>Total budget for this campaign</p>
              </div>
              <div>
                <Input
                  label="Target Metric *"
                ...
                />
                <p className={styles.fieldHint}>Minimum performance to qualify</p>
              </div>
            </div>
            <div className={styles.formGrid}>
              <div>
              ...
              <div className={styles.rewardPreview}>
                <div className={styles.rewardPreviewLabel}>Max Creators Payable</div>
                <div className={styles.rewardPreviewVal}>
                  {form.reward_pool && form.target_reward && Number(form.target_reward) > 0
                    ? Math.floor(Number(form.reward_pool) / Number(form.target_reward))
                    : '—'
                  }
                  <span> creators</span>
                </div>
        ...
        {step === 3 && (
          <div className={styles.formSection}>
            ...
            <p className={styles.stepDesc}>Confirm everything looks right before creating</p>
            <div className={styles.reviewGrid}>
              ...
              <div className={styles.reviewItem}>
                ...
                <div className={styles.reviewVal} style={{ textTransform: 'capitalize' }}>{form.platform}</div>
              </div>
              <div className={styles.reviewItem}>
              ...
              <div className={styles.reviewItem}>
                ...
                <div className={styles.reviewVal}>{form.min_followers || '0'}</div>
              </div>
              <div className={styles.reviewItem}>
                <div className={styles.reviewLabel}>Required Hashtags</div>
                <div className={styles.reviewVal}>{form.required_hashtags || 'None'}</div>
              </div>
              <div className={styles.reviewItem}>
                <div className={styles.reviewLabel}>Banned Keywords</div>
                <div className={styles.reviewVal}>{form.banned_keywords || 'None'}</div>
              </div>
              <div className={styles.reviewItem}>
                <div className={styles.reviewLabel}>Reward Pool</div>
                <div className={styles.reviewVal} style={{ color: 'var(--accent)' }}>${form.reward_pool}</div>
              </div>
              <div className={styles.reviewItem}>
                <div className={styles.reviewLabel}>Target Metric</div>
                <div className={styles.reviewVal}>{Number(form.target_metric).toLocaleString()}</div>
              </div>
              <div className={styles.reviewItem}>
                <div className={styles.reviewLabel}>Per Creator Reward</div>
                <div className={styles.reviewVal} style={{ color: 'var(--accent)' }}>${form.target_reward}</div>
              </div>
            </div>
          </div>
        )}
