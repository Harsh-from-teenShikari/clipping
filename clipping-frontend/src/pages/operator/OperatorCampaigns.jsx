...
export default function OperatorCampaigns() {
  ...
  return (
    <div className={styles.page}>
      <PageHeader
        title="My Campaigns"
        subtitle="Activate, pause and manage all your campaigns"
        action={<Button onClick={() => navigate('/operator/create')}>+ New Campaign</Button>}
      />

      {message && <Alert type="success" style={{ marginBottom: '1rem' }}>{message}</Alert>}
      ...
      {campaigns.length === 0 ? (
        <div className={styles.empty}>
          ...
          <div className={styles.emptyDesc}>Create your first campaign to get started</div>
              ...
              <div className={styles.cardLeft}>
                ...
                <div className={styles.cardInfo}>
                  ...
                  <div className={styles.campMeta}>
                    ...
                    <span>{c.type}</span>
                    <span>·</span>
                    <span className={styles.campReward}>${c.reward_pool?.toLocaleString()} pool</span>
                  </div>
                </div>
              </div>
              ...
              <div className={styles.cardRight}>
                ...
                <div className={styles.actions}>
                  {c.status === 'draft' && (
                    ...
                    >
                      {actionLoading[c.id] === 'activating' ? '...' : '▶ Activate'}
                    </button>
                  )}
                  {c.status === 'active' && (
                    <button
                  ...
                  {c.status === 'paused' && (
                    <button
                      ...
                      disabled={!!actionLoading[c.id]}
                    >
                      {actionLoading[c.id] === 'activating' ? '...' : '▶ Resume'}
                    </button>
                  )}
                  <button
