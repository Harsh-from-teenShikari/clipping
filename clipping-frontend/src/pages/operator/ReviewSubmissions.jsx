      ...
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.modalBody}>
          <div className={styles.subLinkRow}>
            ...
            >
              {sub.post_link?.length > 50
                ? sub.post_link.slice(0, 50) + "..."
                : sub.post_link}
            </a>
          </div>
          ...
          {status === "rejected" && (
            <div className={styles.reasonSection}>
              ...
              {REJECTION_REASONS.map((r) => (
                ...
                >
                  <div
                    className={`${styles.radioCircle} ${reason === r.value ? styles.radioSelected : ""}`}
                  />
                  {r.label}
                </div>
              ))}
      ...
      {campaigns.length === 0 ? (
        <div className={styles.empty}>
          ...
          <div className={styles.emptyDesc}>
            Activate a campaign first to start receiving submissions
          </div>
        </div>
      ) : (
          ...
          <div className={styles.campList}>
            ...
            {campaigns.map((c) => (
              ...
              >
                ...
                <div className={styles.campItemMeta}>
                  {c.platform} · {c.status}
                </div>
              </div>
            ))}
          ...
          <div className={styles.subsPanel}>
            {subLoading ? (
              ...
              <div className={styles.noSubs}>
                ...
                <div className={styles.emptyDesc}>
                  Submissions appear here after passing AI verification
                </div>
              </div>
            ) : (
              <>
                        ...
                        <div className={styles.subCardLeft}>
                          ...
                          <div>
                            ...
                            <a
                              ...
                              className={styles.subLink}
                            >
                              {sub.post_link?.length > 45
                                ? sub.post_link.slice(0, 45) + "..."
                                : sub.post_link}
                            </a>
                          </div>
                        </div>
                ...
                {reviewed.length > 0 && (
                  <div className={styles.subsSection}>
                    ...
                    {reviewed.map((sub) => (
                      <div
                        key={sub.id}
                        className={`${styles.subCard} ${styles.subCardReviewed}`}
                      >
                        <div className={styles.subCardLeft}>
                          <div
                            className={styles.subCreatorAvatar}
                            style={{ opacity: 0.5 }}
                          >
                            {sub.creator_id?.slice(0, 2).toUpperCase()}
                          </div>
                            ...
                            {sub.rejection_reason && (
                              <div className={styles.rejReason}>
                                ...
                                {REJECTION_REASONS.find(
                                  (r) => r.value === sub.rejection_reason,
                                )?.label || sub.rejection_reason}
                              </div>
                            )}
                          </div>
                        </div>
                        <div className={styles.subCardRight}>
                          <Badge
                            color={
                              sub.review_status === "approved" ? "green" : "red"
                            }
                          >
                            {sub.review_status}
                          </Badge>
